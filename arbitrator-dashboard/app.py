...
app = Chalice(app_name="arbitrator-dashboard")
...
def index():
    ...
    # Load the js file for html injection - this breaks with CORS on GCP if you do it in html
    file_js = open("chalicelib/static/js/charts.js","r")  
    charts_js = file_js.read()
    file_js = open("chalicelib/static/js/form_calcs.js","r")  
    ...
    # resp = render_template("index.html", charts_js=charts_js, calcs_js=calcs_js, msg="hello", **curr_data, **h_prices_params, **h_arb_params)
    # # return(resp, 200, headers)

    context = {}
    def render(tpl_path, context):
        path, filename = os.path.split(tpl_path)
        return jinja2.Environment(loader=jinja2.FileSystemLoader(path or "./")).get_template(filename).render(charts_js=charts_js, calcs_js=calcs_js, msg="hello", **curr_data, **h_prices_params, **h_arb_params)

    resp = render("chalicelib/templates/index.html", context)
